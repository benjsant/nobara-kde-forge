        ...
        function renderLaptopChecklist(status, thermal) {
            ...
            // ── Vendor-specific (asusctl, etc.)
            const vendor = status.vendor || {};
            if (vendor.id && vendor.id !== 'other' && (vendor.packages || []).length) {
                const vendorLabel = vendor.name ? esc(vendor.name) : esc(vendor.id.toUpperCase());
                html += '<div class="laptop-group"><div class="laptop-group-title">Specifique vendor : ' + vendorLabel + '</div>'
                    + '<div style="font-size:0.82em;color:var(--text-muted);margin-bottom:6px;">Paquets recommandes pour le materiel detecte.</div>';
                vendor.packages.forEach(pkg => {
                    const done = pkg.installed;
                    const badge = done ? '<span style="font-size:0.75em;background:#d5f4e6;color:#27ae60;padding:2px 6px;border-radius:8px;margin-left:6px;">installe</span>' : '';
                    html += '<label class="laptop-check-item"><input type="checkbox" data-laptop="vendor" value="' + esc(pkg.name) + '" ' + (done ? '' : 'checked') + '>'
                        + '<div><span class="pkg-name">' + esc(pkg.name) + badge + '</span>'
                        + '<span class="pkg-desc">' + esc(pkg.description) + '</span></div></label>';
                });
                html += '</div>';
        ...
        function applyLaptopSelection() {
            ...
            const vendor = [];
            document.querySelectorAll('#laptopChecklist input[data-laptop="vendor"]:checked').forEach(cb => vendor.push(cb.value));

            if (!wantTlp && !monitoring.length && !dock.length && !vendor.length) {
            ...
            if (vendor.length) parts.push(vendor.length + ' vendor');
            ...
            showConfirm('Appliquer la selection laptop ?',
                ...
                () => {
                    fetch('/api/laptop/apply', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({tlp: wantTlp, monitoring: monitoring, dock: dock, vendor: vendor})
                    })
                    .then(r => r.json())
                    .then(data => {
